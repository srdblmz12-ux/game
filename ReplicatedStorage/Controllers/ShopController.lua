...
--// Helper: Viewport Kamera Ayari (3D Önizleme)
local function SetupViewportCamera(viewportFrame, model)
	...
	if head and root then
		...
		-- Esya modelleri için genel odak
...
function ShopController:UpdateCardState(card, details)
	...
	if isOwned then
		card.LayoutOrder = price -- Sahip olunanlari da fiyata göre dizebilirsiniz veya en basa alabilirsiniz
		if isEquipped then
...
function ShopController:OnStart()
	...
	local ShopContainer = ShopHUD:WaitForChild("ShopContainer")
	local SideHUD = PlayerGui:WaitForChild("SideHUD")

	ShopContainer.Interactable = true
	self.CachedData = Net:Invoke("DataService/GetData")

	-- Veri degistiginde (örnegin satin alim sonrasi) güncelle
	...
	-- ShopAssets içindeki modülleri bulma yardimcisi
	...
	-- Kategorileri ve Esyalari Yükle
	for _, CategoryFolder in ipairs(ShopAssets:GetChildren()) do
		local Cards = {}

		-- Klasördeki tanimli esyalar
		...
		-- Oyuncunun sahip oldugu ama listede görünmeyen özel esyalar varsa ekle
